                        ...
                        <div class="col-sm-12 ">
                            ...
                            <div class="card mb-5 ">
                                <div class="card-body m-3">
                                    <div class="pt-4 mb-3">
                                        <h5 class="card-title text-left pb-0 fs-4">Update College Fees</h5>
                                    </div>
                                    @include('shared.success')
                                    @include('shared.error')
                                    <form action="{{ route('fees.update', $fees->id) }}" method="POST" class="row g-3">
                                        @method('PUT')
                                        @csrf
                                        <div class="col-lg-12">
                                            <div class="mb-2">
                                                <label for="yourPassword" class="form-label">Student</label>
                                                <select name="student" class="form-select" disabled>
                                                    <option value="" selected>Choose...</option>
                                                    @foreach ($students as $row)
                                                        <option value="{{ $row->id }}"
                                                            {{ $row->id == $fees->users_id ? 'selected' : '' }}>
                                            ...
                                            <div class="mb-2">
                                                ...
                                                <input type="text" value="{{ $fees->amount }}" name="amount"
                                                    class="form-control">
                                                @error('amount')
                                                    <span class="d-block text-danger fs-6 mt-1">{{ $message }}</span>
                                                @enderror
                                            ...
                                            <div class="mb-2">
                                                ...
                                                <input type="date" name="duedate" value="{{ $fees->due_date }}"
                                                    class="form-control">
                                                @error('duedate')
                                                    <span class="d-block text-danger fs-6 mt-1">{{ $message }}</span>
                                                @enderror
                                            </div>
                                            <input type="hidden" name="status" value="{{ $fees->status }}">
                                            <button class=" mt-3 btn btn-dark w-100" type="submit">Update</button>
                                        </div>
